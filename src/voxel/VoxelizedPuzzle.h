...
public:
    ...
    VoxelizedPuzzle(int nx, int ny, int nz, int joint_id, std::shared_ptr<ColorCoding> coder);

    //array[x][y][z] = true means (x, y, z) is a valid voxel.
    //All valid voxels belong to one part
    VoxelizedPuzzle(int nx, int ny, int nz, int joint_id, const array_3b &array, std::shared_ptr<ColorCoding> coder);

    //array[x][y][z] represents voxel (x, y, z)'s part id
    //-1 means a non-valid voxel
    VoxelizedPuzzle(int nx, int ny, int nz, int joint_id, const array_3i &array, std::shared_ptr<ColorCoding> coder);
