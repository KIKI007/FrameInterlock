...
/* calculate how many part does the assembly has? */
    ...
    for(int ix = 0; ix < Nx; ix++)
    {
        for(int iy = 0; iy < Ny; iy++)
        {
            for(int iz = 0; iz < Nz; iz++)
            {
                int voxel_id = (*voxel_)[ix][iy][iz];
                if(voxel_id >= 0) visited[voxel_id] = true;
            }
        }
    }

    part_num_ = visited.size();

    return part_num_;
}

int VoxelizedInterface::compute_max_part_id() {
    int max_part_id = -1;
    for(int ix = 0; ix < Nx; ix++)
    {
        for(int iy = 0; iy < Ny; iy++)
        {
            ...
            {
                int voxel_id = (*voxel_)[ix][iy][iz];
                if(voxel_id >= max_part_id) max_part_id = voxel_id;
            }
        }
    }
